...
import { filter, from, lastValueFrom, mergeMap, Observable, toArray } from 'rxjs';
...
export class TradingService {
    ...
    async placeMorningSLOrders (): Promise<void> {
        ...
        await this.placeOrders( strategies )
            .then(() => this.logger.log(`${_.map(strategies, 'name')} are executed.`));
    }

    private async placeOrders(strategies: Strategy[]) :Promise<OrderResponseDTO[]> {
        const orderResponses: Observable<OrderResponseDTO[]> = from( this.userService.findAll() ).pipe(
            ...
            mergeMap((orderResponse: OrderResponseDTO[]) => from(orderResponse), 10),
            toArray()
        );
        return await lastValueFrom( orderResponses );
    }

    private async placeOrder ( orderDetail: OrderDetails, holding: HoldingInfoDTO, demat: DematAccount ): Promise<OrderResponseDTO> {
        return await this.tradingFactory.getInstance(demat.broker.name).placeOrder(orderDetail, holding, demat);
    }

    private processData ( historicalData: StockInfoHistorical, currentData: StockInfoMarket, holding: HoldingInfoDTO, strategies: Strategy[] ): OrderDetails | void {
        const orders: OrderDetails[] = [];
        for (let index = 0; index < strategies.length; index++) {
            const { name, mustConditions, mightConditions, mightConditionLimit, orderDetails }: Strategy = strategies[ index ];
            ...
            if ( mustConditions !== undefined ) {
                for ( let i = 0; i < mustConditions.length; i++ ) {
                    ...
                    if ( typeof answer === 'boolean' && answer === false ) {
                        this.logger.log( `${ description } is not satisfaied for ${ holding.tradingsymbol }.This was a part of mustConditions, so breaking the loop.` );
                        shouldProceed = false;
                        break;
                    }
            ...
            if ( !shouldProceed ) {
                this.logger.log( `MustConditions did not satisfy for ${ holding.tradingsymbol }` );
                break;
            }

            if ( mightConditions !== undefined && mightConditions.length != 0 ) {
                const mightConditionsJunkAnswers: (number | boolean)[] = mightConditions.reduce(
                    ( acc: ( number | boolean )[], { filter, description } ) => {
                        ...
                        if ( typeof answer === 'number' || answer ) {
                            ...
                            this.logger.log( `${ description } did not satisfy for ${ holding.tradingsymbol }` );
                        }
                        return acc;
                }, []);
                ...
                if ( mightConditionsJunkAnswers.length < mightConditionLimit ) {
                    shouldProceed = false;
                    this.logger.log( `Only ${mightConditionsJunkAnswers.length} nnumber of conditions are satisfying, so can't proceed`);
                    break;
                }
            }

            if ( shouldProceed ) {
                this.logger.log( `Strategy ${ name } is positive for ${ holding.tradingsymbol } ` );
                orders.push( orderDetails );
            }
        }

        if ( orders.length === 0 ) {
            this.logger.log( `No strategies are currently applicable for ${ holding.tradingsymbol }` );
        }
