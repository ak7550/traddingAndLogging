...
export class TradingService {
    ...
    async handleTradingViewAlert(alert: AlertRequestDTO, dematAccountId?: number): Promise<OrderResponseDTO[] | void> {
        ...
        if (!Number.isNaN(dematAccountId)) {
            ...
            if (demat) {
                ...
                const filteredData: HoldingInfoDTO[] = holding.filter(h => h.tradingsymbol === alert.ticker);
                if (filteredData.length === 0) {
                    this.logger.log(`${alert.ticker} is not present in ${dematAccountId}`);
                    return;
                }
            ...
            const filteredDematsObserVable: Observable<AlertServiceType[]> = from(await this.dematService.findAll()).pipe(
            ...
            filteredAlertService.concat(await lastValueFrom(filteredDematsObserVable));
        }

        // check if the stock qualifies for the alert to take trade
        const [current, historical] = await Promise.all([
            this.stockDataService.getCurrentData(`${alert.exchange}:${alert.ticker}`),
            this.stockDataService.getHistoricalData(`${alert.exchange}:${alert.ticker}`)
        ]);

        const orderResponseObservale: Observable<OrderResponseDTO[]> = from(filteredAlertService).pipe(
            mergeMap(async ({ demat, holding }) => {
                const orderDetail: OrderDetails | void = this.processData(historical, current, holding, strategiesForThisAlert);
                if (orderDetail) {
                    return await this.placeOrder(orderDetail, holding, demat, current, historical);
                }
                return null;
            }, 10), // 10 is the concurrency value
        ...
        const orderResponses: OrderResponseDTO[] = await lastValueFrom(orderResponseObservale);
    ...
    async sellEvery15min(): Promise<OrderResponseDTO[]> {
        ...
        const orderResponses: OrderResponseDTO[] = await this.placeSchedularOrder(strategies)
    ...
    public async closingTimeOrder(): Promise<void> {
        const strategies: Strategy[] = [openHighSellClosingHour, daily21EMARetestBuy, dailyRSIBelow60];
        await this.placeSchedularOrder(strategies).then(() =>
            this.logger.verbose(`${_.map(strategies, "name")} are executed.`)
        );
    }
                    ...
                    .pipe(
                        mergeMap(async (holding: HoldingInfoDTO) => {
                            const [historical, current] = await Promise.all([
                                this.stockDataService.getHistoricalData(`${holding.exchange}:${holding.tradingsymbol}`),
                                this.stockDataService.getCurrentData(`${holding.exchange}:${holding.tradingsymbol}`)
                            ]);
